...
var svg = d3.select("#adj10").append("svg")
	.attr("version","1.1")
    .attr("width", w)
    .attr("height", h)
	.attr("style","background: #112255");
		...
		if (iter <= maxiter) {
			if (front.length > 0) {
				...
				for (ifront = 0; ifront < oldfront.length; ++ifront) {
					...
					var newr = d3.max([c1.r,c2.r])*rand(1.0,0.95);
					if (side < 0.0) {
						var newr = d3.min([c1.r,c2.r])*rand(1.0,0.95);
					}
					if (newr > 0.1) {
						var newnode = circles2tangent(lastcouple[0],"OUT",lastcouple[1],"OUT",newr,side);
						if (!checkcollision(nodes,newnode)) {
							nodes.push(newnode);
							viewbox0 = mergeviewboxes(viewbox0,nodeviewbox(newnode));
					...
					iter++;
					var newviewbox = convertviewboxwidthheight(centerviewbox(expandviewbox(computeviewbox(nodes.slice(0)),1.1), [1,0]));
