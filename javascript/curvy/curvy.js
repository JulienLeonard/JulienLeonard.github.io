...
function fradius(c1,c2) {
    return mymax([c1.r,c2.r])*rand(0.9,0.97);
}

function fcolor(newnode) {
    return myhsla(((newnode.colorindex)%(30))/(30), 1.0, 0.5,1.0);
}
...
function context () {
	...
	if (!checkcollisionquadtree(quadtree,newnodes[0],0.001) && !checkcollisionquadtree(quadtree,newnodes[1],0.001)) {
		...
		var ncolor = myhsla(0.0, 1.0, 0.5, 1.0);
	    ...
	    var newpattern = new bpatternpacking(newnodes,fradius,fcolor);
	...
	// viewviewbox = [200.0,-500.0,1200.0,500.0];
