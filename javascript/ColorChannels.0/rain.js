...
function circlepoints(c,npoints) {
	var result = [];
	...
	for (ia = 0; ia < angles.length; ia++) {
		var ext    = center.add(vrotate(v0,angles[ia]));
		result.push(ext);
	}
	return result;
}
...
function rendercircle(canvas,newnode,ncolor) {
	if (newnode.r < 0.06) {
		...
		var c = circle(newnode.x,newnode.y,newnode.r * 0.5);
		var points = circlepoints(c,3);
		var i = 0;
		for (i = 0;i< points.length;i++) {
			drawcircle(canvas,circle(points[i].x,points[i].y,c.r*0.1),ncolor);
		}
	}
}
