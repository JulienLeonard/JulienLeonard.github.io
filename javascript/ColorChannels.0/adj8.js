...
var svg = d3.select("#adj8").append("svg")
	.attr("version","1.1")
    .attr("width", w)
    .attr("height", h);
		...
		if (iter <= maxiter) {
			if (front.length > 0) {
				...
				for (ifront = 0; ifront < oldfront.length; ++ifront) {
					...
					iter++;
					var newviewbox = convertviewboxwidthheight(centerviewbox(expandviewbox(computeviewbox(nodes.slice(nodes.length-100)),1.1), [1,0]));
