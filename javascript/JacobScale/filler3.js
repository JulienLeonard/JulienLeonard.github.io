...
function initcanvas(w,h,background) {
	var svg = d3.select("#filler3").append("svg")
		.attr("version","1.1")
		.attr("width", w)
		.attr("height", h)
...
function addlights() {
	...
	for (nsubiter = 0; nsubiter < 1; nsubiter++) {
		...
		if (!checkcollisionquadtree(quadtree,newnode)) {
			...
			var mcolor = d3.hsl(0.0, 1.0,1.0);
			
			quadtree.insert(newnode);
			viewbox0 = mergeviewboxes(nodeviewbox(newnode),viewbox0);
			drawcircle(canvas,circle(newnode.x,newnode.y,newnode.r * 0.4),mcolor);
		}
		iter++;
	}
...
function adddust() {
	...
	var nnew = 0;
	for (nsubiter = 0; nsubiter < 1; nsubiter++) {
		var newnode = circle(myrandom(),myrandom(),0.02);
		if (!checkcollisionquadtree(quadtree,newnode)) {
			nodes.push(newnode);
			nnew++;
