...
function fdraw(canvas,pattern,lastindex) {
	var ncolor = myhsla(lastindex/100.0, 1.0, 0.5, 1.0);
	...
	if (arecircletangent(newnode,lastnode)) {
		...
		for (var ic = 0; ic < ccircles.length; ic++) {
			...
			// drawcircle(canvas,new Circle(ccircle.x,ccircle.y,newnode.r/1.25),ncolor);
		}
	}
}
...
function context () {
	...
	var createpattern = function(x,y) {
		...
		if (!checkcollisionquadtree(quadtree,newnodes[0],0.001) && !checkcollisionquadtree(quadtree,newnodes[1],0.001)) {
			...
			var framecenters = circlepoints(framecircle,1000,0.0);
			...
			for (var incircle = 0; incircle < niter-noffset; incircle++) {
				...
				var rsupport = samples(1.0,1.0,ncircle/5);
	...
	function iterframe() {
		...
		// viewviewbox = [-180.0,-150.0,150.0,80.0];
