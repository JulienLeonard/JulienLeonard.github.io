...
function initcanvas(w,h,background) {
	var svg = d3.select("#adj29").append("svg")
		.attr("version","1.1")
		.attr("width", w)
		.attr("height", h)
...
function plug(c1,c2,side,color) {
	return {c1: c1, c2: c2, side: side, color: color}
}
...
var front = [plug(c1,c2,1.0,d3.hsl(0, 1, 0.5)),plug(c2,c1,1.0,d3.hsl(0, 1, 0.5))];
			...
			for (ifront = 0; ifront < oldfront.length; ++ifront) {
				...
				if (newr > 0.0001 * (convertviewboxwidthheight(viewbox0)[2])) {
					...
					if (!checkcollisionquadtree(quadtree,newnode)) {
						...
						var ncolor = d3.hsl(lastcouple.color.h + Math.floor(rand(-5.0,15.0)), 1,0.5);
						...
						front.push(plug(c2,newnode,-side,ncolor));
						if (true) {
							if (true) {
								var newfront = [plug(newnode,c1,-side,ncolor)].concat(front.slice(0));
								front = newfront;
								
								newfront = [plug(c2,newnode,-side,ncolor)].concat(front.slice(0));
								front = newfront;
							}
						}
					}
