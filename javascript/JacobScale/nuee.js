...
function context() {
	...
	var background = Color.prototype.black();

	canvas = initcanvas(canvasname,background);

	var iter = 0;
	var maxiter = 50000000;
	var viewbox0 = [];
	...
	function iterframe() {
		...
		if (iter < maxiter && patterns.length > 0) {
			...
			result = lastpattern.iter(quadtree,canvas,100);
		...
		var viewviewbox = squareviewbox(expandviewbox(viewbox0,1.3));
