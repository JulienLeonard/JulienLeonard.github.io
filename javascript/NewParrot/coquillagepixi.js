...
var canvasname    = "coquillagepixi";
var quadtree      = null;
var lastpatternid = 0;
var patterns    = [];
var canvas = null;
var graphpacket = null;
var graphpacketncircles = 0;

// pixi specifics
var renderer = null;
var stage    = null;
var viewboxpixi = null;

function fdraw(canvas,pattern,lastindex) {
	// var ncolor = myhsla(lastindex/10000.0, 1.0, 0.5, 1.0);
	var factor = 0.1;
	...
	var colorindex = lastindex;
	var ncolor = myhsla((((colorindex+2)%(14100*factor))/(114100*factor)), 1.0, (((colorindex)%(14100*factor))/(14100 * 1.3 *factor)),0.25);

	drawcircle(canvas,cscale(newnode,2.0),ncolor);
	// drawcircle(canvas,cscale(newnode,1.0),myhsla((((colorindex)%(14100*factor))/(114100*factor)), 1.0, (((colorindex)%(14100*factor))/(14100*1.3*factor)),1.0));
	// drawcircle(canvas,cscale(newnode,0.25),Color.prototype.white());

	if (arecircletangent(newnode,lastnode)) {
		var seg = [ccenter(newnode),ccenter(lastnode)];
		var ccircles = linecircles(seg[0],seg[1],3);
...
function context () {
	...
	var createpattern = function(x,y) {
		...
		if (!checkcollisionquadtree(quadtree,newnodes[0],0.001) && !checkcollisionquadtree(quadtree,newnodes[1],0.001)) {
			...
			for (inode = 0; inode < newnodes.length; inode++) {
				...
				drawcircle(canvas,newnodes[inode],myhsla(1.0,0.0,0.0,1.0));
			}
			...
			for (var incircle = 0; incircle < niter-noffset; incircle++) {
				...
				var rsupport = samples(1.0,2.0,ncircle*2);
	...
	function iterframe() {
		...
		var viewviewbox = expandviewbox(viewbox0,1.5);
		
	    viewviewbox = [-300.0,-100.0,200.0,100.0];
