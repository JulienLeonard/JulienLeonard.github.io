...
function context () {
	...
	var touchdrawcircle = function(event) {
		...
		var newnodes = [new Circle(eventcoords[0]-1,eventcoords[1],1.0),new Circle(eventcoords[0]+1,eventcoords[1],1.0)];
		...
		if (!checkcollisionquadtree(quadtree,newnodes[0],0.001) && !checkcollisionquadtree(quadtree,newnodes[1],0.001)) {
			...
			var newpattern = new bpatternbaoswitch(sides,
												   lconcat(samples(1.0,2.0,10),samples(2.0,1.0,10)),
												   function(lastindex) {return d3.hsl(((lastindex)%(28))/(28)*360.0, 1.0,0.7);},
												   newnodes);
	...
	quadtree = new QuadTree(100000,100000);
