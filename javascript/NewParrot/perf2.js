...
function initcanvas(w,h,background) {
	var svg = d3.select("#adj24").append("svg")
		.attr("version","1.1")
		.attr("width", w)
		.attr("height", h)
...
drawcircle(canvas,c2,[255,0,0]);
quadtree.insert(c1);
quadtree.insert(c2);

// var front = [plug(c1,c2,1.0),plug(c2,c1,1.0)];
...
var ifront = 0;
var niterfront = 0;
var maxiterfronts = [500,100,100,100,100,100,100,100];

function iterframe() {
	if (iter <= maxiter) {
		if (front.length == 0 || niterfront >= maxiterfronts[ifront]) {
		...
		if (front.length > 0) {
			...
			for (ifront = 0; ifront < oldfront.length; ++ifront) {
				...
				var newr = mymax([c1.r,c2.r])*0.999;
				var nratio = 0.96;
				if (ifront % 2  != 0) {
					nratio = 1.0;
				}
				...
				if (newr > 0.001 * (convertviewboxwidthheight(viewbox0)[2])) {
					...
					if (true) {
						...
						front.push(plug(c2,newnode,-side));
						if (false) {
							if (ntimer % 5 == 0) {
								if (side > 0.0) {
									var newfront = [plug(newnode,c1,-side)].concat(nextfront.slice(0));
									nextfront = newfront;
								} else {
									var newfront = [plug(c2,newnode,-side)].concat(nextfront.slice(0));
									nextfront = newfront;
								}
