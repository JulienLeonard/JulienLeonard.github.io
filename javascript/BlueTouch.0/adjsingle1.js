...
function initcanvas(w,h,background) {
	var svg = d3.select("#adjsingle1").append("svg")
		.attr("version","1.1")
		.attr("width", w)
		.attr("height", h)
...
function nextcircletangent(c,angle,radius) {
	var x = c.x;
	var y = c.y;
	var r = c.r;
	var c = new Point(x,y);
	var nv = new Point(0.0,radius+r);
	nv.angle = angle * 360.0;
	var newc = c + nv;
	return circle(newc.x,newc.y,radius);
}

function radiusanglesparameters(rfactor,aincr,niter) {
	var result = [];
	var index = 0;
	var radius = 1.0;
	var angle  = 0.0;
...
function plug(c,angle) {
	return {c: c, angle: angle}
}
...
var c1 = circle(0,0,1);
drawcircle(canvas,c1,[255,0,0]);
quadtree.insert(c1);

var front = [plug(c1,0.0)];
nodes.push(c1);
...
var ilevelfront = 0;
var niterfront = 0;
var maxiterfronts = [500,5000,5000,100,100,100,100,100];
	...
	if (iter <= maxiter) {
		...
		if (front.length > 0) {
			...
			for (ifront = 0; ifront < oldfront.length; ++ifront) {
				...
				var c     = lastplug.c;
				var angle = lastplug.angle;
				var newnode = nextcircletangent(c,radius,angle);
