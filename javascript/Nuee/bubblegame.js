...
function fdraw(canvas,pattern,lastindex) {
	...
	// var ncolor = myhsla((bidirection(colorindex,1500))/(1500), 1.0, bidirection(colorindex,121)/121,1.0);
	// var ncolor3 = myhsla((bidirection(colorindex,1500))/(1500), 1.0, bidirection(colorindex,121)/121,0.75);
    // var ncolor = myhsla((((colorindex+2*factor)%(341*factor))/(2141*factor)), 1.0, (((colorindex)%(341*factor))/(341*factor)),1.0);
           var ncolor = myhsla(0.3, 1.0, 0.5,1.0);


	// console.log("fdraw circle",newnode,"ncolor",ncolor);

	// drawcircle(canvas,cscale(newnode,1.0),Color.prototype.black());
	// drawcircle(canvas,cscale(newnode,1.0),ncolor3);
	drawcircle(canvas,cscale(newnode,1.0),ncolor);

	if (arecircletangent(newnode,lastnode) && true) {
		...
		for (var ic = 0; ic < ccircles.length; ic++) {
			...
			// drawcircle(canvas,new Circle(ccircle.x,ccircle.y,newnode.r/5.0),ncolor);
		}
	}
}
...
function context() {
	...
	var createpattern = function(x,y) {
		...
		if (!checkcollisionquadtree(quadtree,newnodes[0],0.001) && !checkcollisionquadtree(quadtree,newnodes[1],0.001)) {
			...
			for (inode = 0; inode < newnodes.length; inode++) {
				...
				var ncolor = myhsla((1%160/(160)), 1.0, 0.5, 1.0);
			...
			var framecircle = new Circle(0.0,0.0,500.0);
			...
			var bside = new BS();
			bside.alts(samples(1,100,100));

			var sides = bside.list();

			var radii = lgeo(5.0,1.0,0.5,100);
			...
			var newpattern = new bpatternbaoswitch([1.0],
						      [1.0],
						      newnodes,
						      fdraw);
	...
	if ( now - timestamp0 > 100/2) {
		...
		result = lastpattern.iter(quadtree,canvas, 1);
	    ...
	    viewviewbox = [-100.0,-100.0,100.0,100.0];
