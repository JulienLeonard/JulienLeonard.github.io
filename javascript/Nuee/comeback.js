...
function fdraw(canvas,pattern,lastindex) {
	...
	var ncolor = myhsla(((lastindex + 12)%(21) + 15)/(21), 1.0, (lastindex+15)%(21)/(21),1.0);
...
function context() {
	...
	function iterframe() {
		...
		if (iter < maxiter && patterns.length > 0) {
			...
			result = lastpattern.iter(quadtree,canvas,10);
