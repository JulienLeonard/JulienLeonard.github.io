...
var svg = d3.select("#adj12").append("svg")
	.attr("version","1.1")
    .attr("width", w)
    .attr("height", h)
		...
		if (iter <= maxiter) {
			if (front.length > 0) {
				...
				for (ifront = 0; ifront < oldfront.length; ++ifront) {
					...
					var newr = d3.max([c1.r,c2.r]);
					if (side < 0.0) {
						var newr = d3.max([c1.r,c2.r])*0.8;
					}
					if (newr > 0.1) {
					...
					iter++;
					var newviewbox = convertviewboxwidthheight(expandviewbox(computeviewbox(nodes.slice(0)),1.1));
