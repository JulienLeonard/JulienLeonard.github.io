		...
		if (iter <= maxiter) {
			if (front.length > 0) {
				...
				for (ifront = 0; ifront < oldfront.length; ++ifront) {
					...
					var c1 = lastcouple[0];
					var c2 = lastcouple[1];
					var newr = c1.r*rand(1.1,0.8);
					var iside;
					for (iside = 0; iside < sides.length; ++iside) {
						var newnode = circles2tangent(lastcouple[0],"OUT",lastcouple[1],"OUT",newr,sides[iside]);
						if (!checkcollision(nodes,newnode)) {
							nodes.push(newnode);
							viewbox0 = mergeviewboxes(viewbox0,nodeviewbox(newnode));
							drawcircle(svg,newnode,d3.rgb(0,0,255));
							front.push([newnode,c2]);
							front.push([c1,newnode]);
						}
					}
					iter++;
					svg.attr("viewBox", convertviewboxwidthheight(expandviewbox(viewbox0,1)).join(" "));
