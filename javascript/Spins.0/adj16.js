...
var svg = d3.select("#adj16").append("svg")
	.attr("version","1.1")
    .attr("width", w)
    .attr("height", h)
		...
		if (iter <= maxiter) {
			if (front.length > 0) {
				...
				for (ifront = 0; ifront < oldfront.length; ++ifront) {
					...
					var newr = d3.max([c1.r,c2.r])*rand(1.05,0.9);
					var nratio = 0.9;
					if (Math.ceil(ntimer / 50) % 3 == 0) {
						nratio = 1.0;
					}
					if (side < 0.0) {
						newr = newr * nratio;
					...
					if (newr > 0.001 * (convertviewboxwidthheight(viewbox0)[2])) {
						...
						if (!checkcollision(nodes,newnode)) {
							...
							if (rand(0.0,1.0) < 0.2 && nratio < 1.0) {
								if (side > 0.0) {
					...
					svg.attr("viewBox", viewviewbox.join(" "));
				}
				ntimer++;
			}
		}
