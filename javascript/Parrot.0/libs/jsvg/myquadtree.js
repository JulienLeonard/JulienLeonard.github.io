...
Quad.prototype.intersect = function(circle) {
	...
	result = false;
	if ((xmin <= (this.xmax + errormargin)) && (xmax >= (this.xmin-errormargin)) && (ymin <= (this.ymax + errormargin)) && ((ymax) >= (this.ymin - errormargin))) {
		result = true;
	}
		...
		if (this.subquads.length) {
			for (var isubquad = 0; isubquad < this.subquads.length; isubquad++) {
				...
				if (subquad.intersect(newcircle)) {
					subresult = subquad.mayintersect(newcircle);
