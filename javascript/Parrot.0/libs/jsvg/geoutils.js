...
function adjcircle(c,r,angle) {
	...
	return circle(newcenter.e(1),newcenter.e(2),r);
}
...
function computecirclesviewbox(circles) {
	...
	var icircle = 1;
	for (icircle = 1; icircle < circles.length; ++icircle) {
...
function checkcirclescollision(circles,circle) {
	...
	for (index = 0; index < circles.length; ++index) {
		...
		var collision = arecirclescolliding(circle,ccircle);
		if (collision) {
			return true;
		}
	}
...
function checkcollisionquadtree(tree,c) {
	...
	for(var j = 0; j < len; j++)
	{
		...
		var colliding = arecirclescolliding(c,item);
