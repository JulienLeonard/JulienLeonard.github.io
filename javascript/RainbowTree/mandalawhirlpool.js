...
function context () {
	var canvasname = "mandalawhirlpool";
	...
	var radiuss = [1.0,1.05,1.1,1.1,1.05,1.0,0.95,0.9,0.9,0.95,1.0];
	...
	function iterframe() {
		if (iter <= maxiter) {
			...
			for (isubiter = 0; isubiter < 100;isubiter++) {
				var newnode = circles2tangent(lastcouple[1],"OUT",lastcouple[0],"OUT",lcircular(radiuss,nodes.length),1.0);
				if (newnode.x != NaN) {
